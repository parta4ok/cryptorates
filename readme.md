...
Миграция - это файл с SQL-командами, который переводит схему базы данных из одного состояния в другое. Вместо того чтобы вручную менять таблицы на каждом окружении, миграции применяются автоматически и воспроизводимо.

**Парные миграции: up и down**

Каждое изменение схемы описывается двумя файлами:

- `.up.sql` - применяет изменение (создать таблицу, добавить колонку)
- `.down.sql` - отменяет его (удалить таблицу, убрать колонку)

```sql
-- 1749457823_create_coins_table.up.sql
...
-- 1749457823_create_coins_table.down.sql
DROP TABLE IF EXISTS crypto.coins;
DROP SCHEMA IF EXISTS crypto;
```

`IF EXISTS` и `IF NOT EXISTS` - обязательны. Без них повторный запуск миграции упадёт с ошибкой.
Префикс определяет порядок применения миграций. Нельзя просто нумеровать с 1 - в команде два разработчика одновременно создадут `2_...` и порядок нарушится.

Правильный подход - Unix timestamp в качестве префикса. Он уникален и отражает реальный порядок создания:
...
Итоговое имя файла: `1749457823_create_coins_table.up.sql`.

Можно использовать разные инструменты для накатывания миграций, автор рекомендует [golang-migrate](https://github.com/golang-migrate/migrate)

### Почему следует выбирать пул соединений?

Одно соединение с базой данных - это один канал. Все горутины выстраиваются в очередь и ждут своей очереди. При любой нагрузке это мгновенно становится узким местом.

Пул управляет набором соединений и выдаёт свободное на каждый запрос:

```go
db, err := pgxpool.New(ctx, connString)
```

`pgxpool` сам следит за здоровьем соединений, переподключается при обрывах и возвращает соединения в пул после использования. Одиночное соединение (`pgx.Connect`) уместно только для миграций или одноразовых скриптов - там параллелизм не нужен.

### Для чего следует предусматривать метод закрытия соединения

Пул держит открытые соединения с базой до тех пор, пока их явно не освободят. При остановке сервиса без закрытия пула - соединения зависают на стороне postgres до таймаута.
...
`sync.Once` гарантирует что `Close` безопасно вызывать несколько раз - повторный вызов ничего не сломает. Метод закрытия регистрируется при инициализации приложения и вызывается в момент graceful shutdown.

### Как работать с батчами

Если нужно вставить несколько записей - отправлять каждую отдельным запросом расточительно: каждый вызов это отдельный round trip до базы и обратно.

Батч собирает все операции и отправляет одним пакетом:

```go
batch := &pgx.Batch{}
for _, coin := range coins {
    batch.Queue(
        `INSERT INTO crypto.coins (title, cost, actual_at) values ($1, $2, $3)`,
        coin.Title(),
        coin.Cost(),
        coin.ActualAt(),
    )
...
10 монет без батча - 10 round trip'ов. С батчем - один. При росте нагрузки разница становится принципиальной.

### Почему важно обрабатывать rows.Error?

`rows.Next()` возвращает `false` по двум причинам: строки закончились, либо в процессе итерации произошла ошибка - например, соединение оборвалось. Различить их без явной проверки невозможно.
...
if rows.Err() != nil {
    return nil, errors.Wrapf(entities.ErrInternal, "process rows err: %v", rows.Err())
}
```

Без этой проверки функция вернёт частичный результат как будто всё прошло успешно. Это один из самых тихих источников багов при работе с базой.

### Идентификация внешней ошибки

Внешние библиотеки возвращают свои типы ошибок - они не должны просачиваться наружу из адаптера. Слой выше не должен знать что используется `pgx`, `lib/pq` или любая другая библиотека.

Задача адаптера - поймать внешнюю ошибку и перевести её в доменную:

```go
if errors.Is(err, pgx.ErrNoRows) {
    return nil, errors.Wrap(entities.ErrNotFound, "no rows found")
}
```

Если внешняя ошибка уйдёт наверх без трансляции - `cases` слой столкнётся с типом из `pgx`, что нарушает принцип инверсии зависимостей: бизнес-логика не должна знать о деталях хранилища. Sentinel-ошибки домена - единственное что должно пересекать границу адаптера.

### Как защититься от SQL-инъекций

SQL-инъекция - это когда пользовательские данные попадают напрямую в текст запроса и интерпретируются как часть SQL, а не как значение.

Представь, что title приходит от пользователя и запрос собирается так:

```go
query := "SELECT * FROM crypto.coins WHERE title = '" + title + "'"
```

Если пользователь передаст `' OR '1'='1` - запрос вернёт все строки таблицы. Если передаст `'; DROP TABLE crypto.coins; --` - таблица будет удалена.

Решение - плейсхолдеры. Значения передаются отдельно от текста запроса, база данных сама обеспечивает их безопасную подстановку:
...
```go
query := `SELECT * FROM crypto.coins WHERE title = ANY($1)`

rows, err := storage.db.Query(ctx, query, titles)
```

`$1`, `$2`, `$3` - это плейсхолдеры. Драйвер `pgx` передаёт значения отдельным параметром, и база данных никогда не перепутает данные с командой. Это правило без исключений: любые внешние данные в запросе - только через плейсхолдеры.
...
### Работа с DTO и почему нельзя давать аннотации объектам слоя entities даже если очень хочется?

`entities.Coin` - это доменный объект. Он не знает ничего о HTTP, JSON или любом другом протоколе передачи данных. Если добавить к нему JSON-теги, домен начнёт зависеть от деталей транспортного слоя - это нарушение принципа инверсии зависимостей.

Кроме того, поля `Coin` приватные. JSON-маршалинг работает только с экспортируемыми полями, так что теги всё равно не сработают без дополнительных ухищрений.
...
Порт конвертирует доменные объекты в DTO перед отправкой клиенту:
...
Если завтра API потребует переименовать поле `actual_at` в `updated_at` - меняется только DTO, доменный объект не трогается.

### Middleware что это такое и зачем нужны?

Middleware - это обёртка вокруг HTTP-обработчика, которая выполняет сквозную логику до или после его вызова. Одно middleware применяется ко всем маршрутам сразу, без дублирования кода в каждом обработчике.
Типичные задачи для middleware: аутентификация, логирование, CORS, rate limiting, восстановление после паники. Всё это пишется один раз и автоматически применяется к каждому маршруту.

### Определение кода ответа, почему важно распозновать ошибки?

HTTP-статус - это не технический детерминизм, это контракт с клиентом. `400` говорит «ты отправил некорректные данные», `404` - «ресурс не найден», `500` - «проблема на нашей стороне». Клиент строит своё поведение на основе этих кодов.

Именно для этого sentinel-ошибки пробрасываются через все слои без потери идентичности - чтобы на уровне порта их можно было точно распознать:

```go
switch {
case errors.Is(err, entities.ErrInvalidParam):
    errDTO.Status = http.StatusBadRequest   // 400
case errors.Is(err, entities.ErrNotFound):
    errDTO.Status = http.StatusNotFound     // 404
default:
    errDTO.Status = http.StatusInternalServerError // 500
}
```

Если вместо sentinel-ошибок использовались бы строки или нетипизированные `errors.New` в каждом методе - здесь пришлось бы угадывать тип ошибки по тексту. Это хрупко и ломается при первом же рефакторинге.

### Сваггер спецификация, зачем она?

Swagger (OpenAPI) - это формализованное описание API: какие эндпоинты существуют, что принимают, что возвращают. Генерируется автоматически из аннотаций прямо в коде:
...
Из этих аннотаций утилита `swag` генерирует JSON-спецификацию, которая превращается в интерактивную документацию - фронтенд-разработчик может отправлять запросы прямо из браузера, не читая Go-код.

Главная ценность: спецификация живет рядом с кодом и обновляется вместе с ним. Отдельный документ в Notion или Confluence устаревает мгновенно - спецификация из аннотаций всегда актуальна.
...
### Работа с конфигурациями, как передать путь до файла с конфигуацией

Путь до файла конфигурации передаётся через флаг командной строки. Это стандартная практика для серверных приложений - путь может отличаться в зависимости от окружения (dev, staging, prod):
...
тогда запуск выгдядит так:

```sh
./appName --config=path_to_config_file
```

Флаг предпочтительнее захардкоженного пути: бинарник не знает из какой директории его запустят. Окружение (dev/prod) определяет оператор при запуске, а не разработчик при компиляции.

Секреты - отдельная история: они не попадают в файл конфигурации, а передаются через переменные окружения (`API_KEY`). Файл конфигурации может попасть в репозиторий, переменная окружения - нет.

### Структура VS гибкий подход к парсингу конфигурационного файла

**Подход через структуру** (viper, envconfig): поля конфигурации описываются явно в виде struct с тегами. Компилятор проверяет типы, IDE подсказывает поля.
...
Минус: при добавлении нового поля в yaml нужно обновить структуру.

**Гибкий подход** (koanf): конфиг читается как произвольное дерево ключей. Доступ к значениям через dot-нотацию:
...
Плюс: можно строить ключ динамически (как `StorageConnstr` - тип хранилища подставляется в путь). Добавление нового раздела в yaml не требует изменения кода парсера.

В проекте используется koanf - именно из-за динамического ключа для connection string: тип хранилища (`pg`, `redis` и т.д.) определяет, какой раздел читать.

### Построение application

`App` в проекте - это оркестратор: он создаёт все зависимости и связывает их вместе. Инициализация разбита на отдельные методы, каждый отвечает за один компонент:
...
`BaseApplication` собирается через builder-паттерн - каждый компонент сам регистрирует свои функции запуска и остановки:
...
`starters` - функции, которые запускают компоненты. `waiters` - функции, которые их корректно останавливают. Разделение намеренное: запуск происходит синхронно по порядку, остановка - тоже по порядку, но с контекстом таймаута.

### Встраивание GracefulShutdown

Без graceful shutdown процесс умирает мгновенно: обрываются in-flight запросы, незавершённые транзакции, незакрытые соединения с БД.

Механизм - подписка на сигналы ОС:

```go
osMon := make(chan os.Signal, 1)
signal.Notify(osMon, syscall.SIGINT, syscall.SIGTERM)
<-osMon // блокируемся до получения сигнала
slog.Info("shutdown signal received")
```

`SIGINT` - Ctrl+C в терминале. `SIGTERM` - сигнал от оркестратора (Kubernetes, systemd) при остановке контейнера.

После получения сигнала запускается остановка с таймаутом - компонентам даётся ограниченное время завершить работу:
...
Каждый `waiter` получает этот контекст и должен уложиться в отведённое время. Если не уложился - контекст отменяется и процесс завершается принудительно.
